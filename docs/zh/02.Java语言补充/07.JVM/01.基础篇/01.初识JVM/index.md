...
| 类型         | 优点                                                         | 缺点                                                         |
| ------------ | ------------------------------------------------------------ | ------------------------------------------------------------ |
| 手动内存管理 | 程序员完全控制内存分配和释放的时机。<br>没有垃圾回收器的性能开销。<br/>更适合系统级编程和性能敏感的应用。<br/>内存使用效率高，可以精确控制内存布局。<br/>实时性好，没有垃圾回收暂停。<br/>可以管理各种资源，如：文件、网络连接等。<br>运行时性能更高，响应时间一致性好。 | 容易出现内存泄漏（忘记释放内存）。<br>可能发生悬空指针（使用已释放的内存）。<br/>双重释放错误（释放同一块内存两次）。<br/>需要更多的编程经验和仔细的设计。<br/>开发效率低，需要额外时间处理内存管理。<br/>调试困难，内存相关 bug 难以定位。<br/>学习成本高，需要深入理解内存管理机制。<br/>维护成本高，代码重构时容易引入错误。 |
| 自动内存管理 | 编程简单，无需手动管理内存，专注业务逻辑。<br/>自动避免内存泄漏、悬空指针等问题。<br/>开发效率高，减少内存管理代码，加快开发速度。<br/>代码简洁，易于阅读和维护。<br/>学习成本低，初学者更容易上手。<br/>类型安全，通常配合类型安全的语言特性。<br/>调试相对简单，减少内存相关错误。<br/>团队协作效率高，降低对程序员技能要求。 | 性能开销，垃圾回收器消耗 CPU 和内存资源。<br/>暂停时间，垃圾回收时可能造成程序暂停。<br/>内存使用多，通常使用更多内存，存在内存开销。<br/>控制受限，无法精确控制内存分配和释放时机。<br/>不适合实时系统，垃圾回收的不确定性。<br/>调优复杂，垃圾回收器的调优需要专业知识。<br/>可能存在循环引用问题。<br/>启动时间可能较慢（JIT 编译等）。 |
